...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        expected_keys = {f"{x}-{y}" for x in range(1, 31) for y in range(1, 4)}
        ...
        if missing_keys:
            self.stdout.write(self.style.WARNING(f"缺失以下记录编号，共{len(missing_keys)}条:"))
        ...
        for item in data:
            try:
                if ProductInfo.objects.filter(image_id=item.get('id')).exists():
                    ProductInfo.objects.filter(image_id=item.get('id')).update(
                        name=item.get('name', ''),
